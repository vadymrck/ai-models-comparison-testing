...
def test_temperature_impact_on_accuracy(
    ...
    print(f"\n  Testing temperature: {temperature_value}")
    print(f"  {'='*60}")
...
def test_batch_processing(openai_client, sentiment_dataset):
    ...
    for batch_idx in range(num_batches):
        ...
        if len(predicted_labels) < len(batch_cases):
            print(".  Warning: Fewer labels than reviews in this batch!")
    ...
    print(f"\n   Total time: {total_time:.2f}s for {total_cases} reviews")
    print(f"    Average per review: {total_time/total_cases:.3f}s")
